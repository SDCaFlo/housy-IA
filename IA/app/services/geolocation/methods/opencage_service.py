...
def forward_search(search_location:str , area:str = 'Lima Metropolitana', countrycode:str ='PE', proximity:dict = {'lat': -12.0464 ,'lon': -77.0428})->tuple[str,dict]:
    ...
    formatted_name = best_result.get('formatted') #.i.e.: 'San Bartolo, Lima Metropolitana, Perú'
    coordinates = best_result.get('geometry')   #result format:{'lat': -12.389822, 'lng': -76.7805897}

    coordinates['lon'] = coordinates['lng'] # aqui agregamos LON para hacer match con nuestro formato
    coordinates.pop('lng')
