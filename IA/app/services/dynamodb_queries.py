...
from app.models.ChatMessage import ChatMessage, ChatHistoryElement
...
def get_all_messages(primary_key: str):
    ...
    response = get_dynamodb_client().query(
        ...
        ExpressionAttributeValues={":pk_val": {"S": primary_key}},
...
def write_message(table_name: str, serialized_item):
    get_dynamodb_client().put_item(TableName=table_name, Item=serialized_item)
    return None
...
def message_wrapper_flex(message_dict: dict):
    ...
    return {**message_dict, "SK": "TIMESTAMP#" + get_current_timestamp()}
